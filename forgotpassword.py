...
class Fp(Tk):
    def __init__(self):
        ...
        def ins():
            if (len(d.get())) < 8 or len(e.get()) < 8:
                ...
                try:
                    ...
                    self.myCursor.execute("Update User_login set password = ? where username = ?",[e.get(),a.get()])
                    self.conn.commit()
                    self.myCursor.close()
                    self.conn.close()
        ...
        def check():
            if len(a.get()) < 5:
                ...
                try:
                    self.conn = sqlite3.connect('Bookstore.db')
                    self.myCursor = self.conn.cursor()
                    self.myCursor.execute("Select username,ques,ans from User_login where username = ? and ans = ? ",[a.get(),c.get()])
        ...
        #label and input box
        Label(self, text="Enter User name", font=('arial', 15, 'bold')).place(x=40, y=20)
        ...
        ttk.Combobox(self, textvariable = b,values=["What is your school name?", "What is your pet name?","What is your bike name?"], width=37,state="readonly").place(x=230, y=74)
